...
export default function DocumentsPage() {
    ...
    const handleDeleteDocument = async (doc: Document) => {
        ...
        try {
            ...
            addToast({ type: 'error', message: err.message || 'فشل حذف الوثيقة' });
        } finally {
            setDeletingId(null);
        }
    };
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">إجمالي الوثائق</div>
                </div>
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                    <div className="text-3xl font-bold text-red-600">{stats.obsolete}</div>
                    <div className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1">
            ...
            {loading ? (
                ...
                <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
                                ...
                                return (
                                    <tr key={doc.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={cn("px-2 py-1 rounded-full text-xs font-medium", typeConfig.color === 'blue' && "bg-blue-100 text-blue-700")}>
                                                {typeConfig.label}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => navigate(`/documents/${doc.id}`)} className="text-slate-400 hover:text-primary-600" title="عرض">
                                                    <EyeIcon className="w-5 h-5" />
                                                </button>

                                                {can('documents', 'edit') && (
                                                    <button onClick={() => { setEditingDocument(doc); setShowFormModal(true); }} className="text-slate-400 hover:text-primary-600" title="تعديل">
                                                ...
                                                <button
                                                    ...
                                                    className="text-slate-400 hover:text-gray-700 dark:hover:text-gray-200"
                                                    title="طباعة"
                                                >
                                                    {printingId === doc.id ? (
                                                ...
                                                {((doc.status === 'approved' && can('documents', 'archive')) ||
                                                    (doc.status !== 'approved' && can('documents', 'delete'))) && (
                                                        <button onClick={() => handleDelete(doc.id)} className="text-slate-400 hover:text-red-600" title={doc.status === 'approved' ? 'أرشفة' : 'حذف'}>
                                                            {doc.status === 'approved' ? <ArchiveBoxIcon className="w-5 h-5" /> : <TrashIcon className="w-5 h-5" />}
                                                        </button>
                                                    )}
                                            </div>
                                        </td>
