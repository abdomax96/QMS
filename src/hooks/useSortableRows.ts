...
export type RowSorter<T> = (left: T, right: T) => number;
...
export const compareTextValues = (left: unknown, right: unknown) =>
  compareWithEmptyLast(left, right, (leftValue, rightValue) => collator.compare(String(leftValue), String(rightValue)));

export const compareNumberValues = (left: unknown, right: unknown) =>
  compareWithEmptyLast(left, right, (leftValue, rightValue) => normalizeNumber(leftValue) - normalizeNumber(rightValue));

export const compareDateValues = (left: unknown, right: unknown) =>
  compareWithEmptyLast(left, right, (leftValue, rightValue) => normalizeDate(leftValue) - normalizeDate(rightValue));

export const compareBooleanValues = (left: unknown, right: unknown) =>
  Number(Boolean(left)) - Number(Boolean(right));

export const createTextSorter = <T>(selector: (row: T) => unknown): RowSorter<T> => (left, right) =>
  compareTextValues(selector(left), selector(right));

export const createNumberSorter = <T>(selector: (row: T) => unknown): RowSorter<T> => (left, right) =>
  compareNumberValues(selector(left), selector(right));

export const createDateSorter = <T>(selector: (row: T) => unknown): RowSorter<T> => (left, right) =>
  compareDateValues(selector(left), selector(right));

export const createBooleanSorter = <T>(selector: (row: T) => unknown): RowSorter<T> => (left, right) =>
  compareBooleanValues(selector(left), selector(right));

interface UseSortableRowsOptions<T> {
  rows: readonly T[];
  sorters: Record<string, RowSorter<T>>;
...
export const useSortableRows = <T>({
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...rows].sort((left, right) => (sortDirection === 'asc' ? sorter(left, right) : sorter(right, left)));
